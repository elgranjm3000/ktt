...
class MantixOaD {
    ...
    function MantixOaD() {
        ...
        $this->con = mysql_connect("localhost","root","123456") or die(mysql_error());
        $this->db =  mysql_select_db("ktz", $this->con);
    }
        ...
        for ($a=2;$a<=$this->total;$a++) {
            ...
            if($this->cols[$a]->marcado==1) {
...

                {
                    ...
                    if((strcmp($this->tabla, "mor_clientes")== 0))

                    {

                        if(strcmp($this->cols[$a]->nombre, "password")!= 0)
    ...
    function toggle_estado_actualizar_estado()

    {

        if($_POST["accion_activa"]==0)

        {

            $r = $this->ejecutar("update ".$this->tabla." set estado=0 where id = " .$_POST["idobj"]);
...

        {
            ...
            $r = $this->ejecutar("update ".$this->tabla." set estado=1 where id = " .$_POST["idobj"]);
