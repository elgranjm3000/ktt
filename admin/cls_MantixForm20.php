...
class MantixForm {
    ...
    function ver()
    {
        ...
        if ($_POST["accion"]==20 || $_POST["accion"]==2 || $_POST["accion"]==21)
        {
            $idobj=$_POST["idobj"];
        ...
        $accionform=($this->atributos["accionform"]?$this->atributos["accionform"]:$_SERVER["REQUEST_URI"]);
        ...
        $cadf.='<input id="accion_extra" name="accion_extra" type="hidden">'."\r\n";

        if(strcmp($this->atributos["var_ayuda"], "pedidos")== 0)
        {
            $cadf.='<input id="accion" name="accion" type="hidden" value="53">'."\r\n";
            $cadf.='<input id="id_clientes" name="id_clientes" type="hidden" value="53">'."\r\n";
        }
        else
        {
        ...
        $cadf.='<legend align="center">'.$tit_form.'</legend>'."\r\n";

        if(isset($this->atributos["texto_idiomas"]))
        {
            $cadf.='<div id="btn_idiomas" style="width: 390px; height: 20px; margin: auto;">';
            $cadf.='<input type="button" value="Espa&ntilde;ol" class="form_submit" id="btn_esp" style="margin-left:1px; background-color: #B163A3; " />';
            $cadf.='<input type="button" value="Ingl&eacute;s" class="form_submit" id="btn_ing" style="margin-left:1px; background-color: #B163A3;" />';
            $cadf.= '</div>';
        }
        ...
        for($a=0;$a<$total;$a++)
        {
            ...
            switch($this->controles[$a]["tipo"])
            {
                ...
                default:
                    ...
                    $controles.='</div><br style="clear:both" id="br_'.$this->controles[$a]["campo"].'"/>'."\r\n";
        ...
        switch($this->atributos["mostrar_submit"])
        {
            ...
            case "2": if($_POST)
                    {
                        $cadf.='<input name="Submit" type="button" class="form_submit" value="Actualizar '.$this->atributos["texto_submit"].'"  onClick="validar()" />';
                    }
                    break;
            case "0": $cadf.="";
                break;
            default:
                ...
                /** reemplazar el texto de los botones con nombre predeterminado(crear, actualizar) - agregado para el reporte de eventos */
                if(isset($this->atributos["mostrar_vrd"]))
                    $pre_sub = "Visualizar ";
                /** ********************************** */
                ...
                {
                    if( ($this->atributos["texto_submit"] != "Reporte") && ($this->atributos["texto_submit"] != "Reporte General de Ventas") && ($pre_sub != "Crear ") )
                    {
                        $cadf .='<input name="Submit" type="button" class="form_submit" value="'.$pre_sub.$this->atributos["texto_submit"].'" onClick="muestra_info()">';
                    }
                    else
                    {
                ...
                {
                    $cadf.='<input name="Submit" type="button" class="form_submit" value="'.$pre_sub.$this->atributos["texto_submit"].'" onClick="validar()" id="btn_actualizar">';
                    //$cadf .='<input type=hidden name=id_obj id=id_obj value='.$_POST['idobj'].'';
                }

                //agregamos un boton auxiliar para validad las accciones del principal
                $cadf.='<input name="Submit" type="button" class="form_submit" value="'.$pre_sub.$this->atributos["texto_submit"].'" id="btn_auxiliar" style="display:none">';
                ...
                if($pre_sub=="Actualizar ")
                {
                    ...
                    if(isset($this->atributos["mostrar_vp"]))
                    {
                        if($this->atributos['tipo_vp'] == "Post_1")
                            $cadf.='<input type="button" value="Mostrar '.$this->atributos["mostrar_vp"].'" class="form_submit" onClick="grid_modificar('.$_SESSION['id_comision'].');" style="margin-left:1px;" />';
                        if($this->atributos['tipo_vp'] == "Dev")
                            $cadf.='<input type="button" value="Mostrar '.$this->atributos["mostrar_vp"].'" class="form_submit" onClick="grid_modificar('.$_SESSION['id_devolucion'].');" style="margin-left:1px;" />';
                    }

                    //cambia el nombre en configuraciones
                    //$cadf.='<input type="button" value="Volver al listado" class="form_submit" onclick="window.location.href=\''.$ruta.'?id=1'.'\'" style="margin-left:1px;" />';
                    $cadf.='<input type="button" value="Volver al listado" class="form_submit" onclick="window.location.href=\''.$ruta.'?id='.$_GET["id"].'\'" style="margin-left:1px;" />';
                }
        }
    ...
    function get_opciones_id($tabla,$campo,$ordenars,$ids,$id_asoc='0',$where)
    {
        $ta=new MantixOaD();
        $ordenar="";
        ...
        if($ids!="" || 1==1)
        {
            $ta->listaSP("select * from ".$tabla.$where.$ordenar,"","");
            $campos=explode("+",$campo);
    ...
    function do_transfers($tabla,$campo,$id_asoc,$values)
    {
        ...
        $lista=array();
        $ta=new MantixOaD();
        $res=$ta->ejecutar("select $id_asoc, $campo from $tabla where formato like '240 x 306'");
        while($r=mysql_fetch_assoc($res))
        {
            if(count($vals)>0)
            {
                ...
                {
                    $lista[0].='<label class="trans_li2 trans_li2_r"><img src="../tumber.php?w=140&h=140&src=../images/recursos/'.$r[$campo].'"/><input type="checkbox" value="'.$r[$id_asoc].'" /></label>';
                }
            }
            else
            {
    ...
    function control($ctl)
    {
        ...
        switch ($tipo)
        {
            case "hidden": $cad='<input type="hidden"  id="'.$ctl["campo"].'" name="'.$ctl["campo"].'" value="'.$valor.'" />';
                break;
            case "text":
                $cad='<input id="'.$ctl["campo"].'" name="'.$ctl["campo"].'" type="'.$tipo.'" class="'.$css_campo.'" maxlength="' .$max_car.'" '.$ctl["extras"].' value="'.$valor.'" autocomplete="off" title="'.$ctl["title"].'" /><input type="hidden"  id="'.$ctl["campo"].'_ant" name="'.$ctl["campo"].'_ant" value="'.$valor.'">
                <script type="text/javascript">mostrar_tooltip("'.$ctl["campo"].'")</script>
                '.$ast;
                break;
            /*case "text_tooltip":
                $cad='<input id="'.$ctl["campo"].'" name="'.$ctl["campo"].'" type="'.$tipo.'" class="'.$css_campo.'" maxlength="' .$max_car.'" '.$ctl["extras"].' value="'.$valor.'" autocomplete="off" title="aa"/><input type="hidden"  id="'.$ctl["campo"].'_ant" name="'.$ctl["campo"].'_ant" value="'.$valor.'">
                <script type="text/javascript">mostrar_tooltip("'.$ctl["campo"].'")</script>
                '.$ast;
                break;*/
            case "readonly": $cad='<input id="'.$ctl["campo"].'" name="'.$ctl["campo"].'" type="text" class="'.$css_campo.'" maxlength="' .$max_car.'" '.$ctl["extras"].' value="'.$valor.'" readonly=""><input type="hidden"  id="'.$ctl["campo"].'_ant" name="'.$ctl["campo"].'_ant" value="'.$valor.'">'.$ast;
                break;
            case "disabled": $cad='<input id="'.$ctl["campo"].'" name="'.$ctl["campo"].'" type="text" class="'.$css_campo.'" maxlength="' .$max_car.'" '.$ctl["extras"].' value="'.$valor.'" disabled="disabled"><input type="hidden"  id="'.$ctl["campo"].'_ant" name="'.$ctl["campo"].'_ant" value="'.$valor.'">'.$ast;
                break;
            case "listadoble":
            ...
            case "archivogg":
                $cad.='
                    <input type="text" value="'.$valor.'" name="'.$ctl["campo"].'" id="'.$ctl["campo"].'" class="form_input"  class="'.$css_campo.'" '.$ctl["extras"].'/>
            ...
            case "area":
                if($ctl["title"] != "")
                    $cad='<textarea id="'.$ctl["campo"].'" name="'.$ctl["campo"].'" class="form_area"'.$ctl["extras"].' title="'.$ctl["title"].'a">'.$valor.'</textarea><script type="text/javascript">mostrar_tooltip("'.$ctl["title"].'")</script>'.$ast;
                else
                    $cad='<textarea id="'.$ctl["campo"].'" name="'.$ctl["campo"].'" class="form_area"'.$ctl["extras"].'>'.$valor.'</textarea>'.$ast;
                break;
            case "checkbox": $cchk="";
                $ant_v=0;
                if ($valor!="")
            ...
            /*case "hora":
                $cad= '<input size="10" id="'.$ctl["campo"].'" class="form_date" type="text" name="'.$ctl["campo"].'" title=""  value="'.$valor.'"/>'.$ast.'<input type="hidden" name="'.$ctl["campo"].'_ant value="'.$valor.'">'."\n";
                //$cad.='<script type="text/javascript">$(document).ready(function(){$("#'.$ctl["campo"].'").timepicker();});</script>';
                break;*/

            case "fck":
                //$cad='<div><textarea id="'.$ctl["campo"].'" name="'.$ctl["campo"].'" style="display:none">'.$valor.'</textarea><iframe id="'.$ctl["campo"].'___Frame" src="fckeditor/editor/fckeditor.html?InstanceName='.$ctl["campo"].'" width="734" height="400" frameborder="0" scrolling="no"></iframe></div><br>';
                $cad='<div><textarea id="'.$ctl["campo"].'" name="'.$ctl["campo"].'" style="display:none">'.$valor.'</textarea><iframe id="'.$ctl["campo"].'___Frame" src="fckeditor/editor/fckeditor.html?InstanceName='.$ctl["campo"].'" width="734" height="250" frameborder="0" scrolling="no"></iframe></div><br>';
                break;
            case "fck1":
            ...
            case "select":
                if($ctl["tabla_asoc"]!="")
                {
                    $ops=$this->get_opciones_id($ctl["tabla_asoc"],$ctl["campo_asoc"],$ctl["ordenar"],$valor,$ctl["id_asoc"],$ctl["where"]);
                }
                else {
                    $ops=$ctl["opciones"];
                }
            ...
            case "select_read":
                ...
                if($valor!="") $ops=str_replace('value="'.$valor.'"','value="'.$valor.'" selected="selected" ',$ops);
                $cad= '<select id="'.$ctl["campo"].'" name="'.$ctl["campo"].'" class="form_select" '.$ctl["extras1"].' onmouseover="this.disabled=true;" onmouseout="this.disabled=false;">'.$ops.'</select><input type=hidden name="'.$ctl["campo"].'_ant" value="'.$valor.'">'.$ast;
                break;
            case "select_multiple":
                ...
                if($valor!="")
                    ...
                    {
                        //$cad.= '<select id="'.$ctl["campo"].'" name="'.$ctl["campo"].'[]" class="form_select_multiple" '.$ctl["extras1"].' multiple>';

                        $cadena = explode(", ", $valor);
                        foreach($cadena as $campo)
                        {
                            $ops.=str_replace('value="'.$campo.'"','value="'.$campo.'" selected="selected" ',$ops);
                        }

                        //$cad.= $ops;

                        //$cad.= '</select><input type=hidden name="'.$ctl["campo"].'_ant []" value="'.$valor.'">'.$ast;
                    }

                $cad= '<select id="'.$ctl["campo"].'" name="'.$ctl["campo"].'[]" class="form_select_multiple" '.$ctl["extras1"].' multiple>'.$ops.'</select><input type=hidden name="'.$ctl["campo"].'_ant []" value="'.$valor.'">'.$ast;
    ...
    function ver_js()
    {
        ...
        $r.="var res = comprueba_evaluacion();"."\n";
        $r.="if(res==0) {return 0;}"."\n";
        $r.="var f = document.formulario; var res_valida=true;" ."\n";
        $r.=$this->js ;
        $r.='if(res_valida)  {verifica_galeria("'.$_SERVER["REQUEST_URI"].'"); f.submit();} else {  $("#linkalerta").click();  }'."\n" ;
        $r.="}" ."\n";
