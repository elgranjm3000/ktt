...
class MantixGrid {
    ...
    function mostrar_buscador() {
        if($this->atributos["ver_buscador"]=="1") {
            ...
            for($a=0; $a<=count($this->buscador)-1;$a++) {
                ...
                switch($tipo) {
                    case "text" : $r.='<input class="form_input" type="input" name="bus_'.$this->buscador[$a]["id"].'" id="bus_'.$this->buscador[$a]["id"].'" value="'.$_POST["bus_".$this->buscador[$a]["id"]].'" autocomplete="off" />';
                        break;
                    case "select":
                    ...
                    case "fecha":
                        ...
                        $r.= '<input size="10" id="bus_'.$this->buscador[$a]["id"].'" class="form_date" type="text" READONLY name="bus_'.$this->buscador[$a]["id"].'" value="'.$_POST['bus_'.$this->buscador[$a]["id"]].'" title="DD/MM/YYYY" > ';
                        $r.='<script type="text/javascript">$(document).ready(function(){$("#bus_'.$this->buscador[$a]["id"].'").datepicker(configdatetime);});</script>';
                        break;
                }
                ...
                if($_POST['bus_'.$this->buscador[$a]["id"]]!="") {
                    ...
                    switch($tipo) {
                        ...
                        case "select" : $cad.= $this->buscador[$a]["campo"]."='".$_POST['bus_'.$this->buscador[$a]["id"]]."'";
                            break;
                        case "fecha" :
                            list($dia,$mes,$year)=explode("/",$_POST['bus_'.$this->buscador[$a]["id"]]);
                            $fecha="$year-$mes-$dia";
                            $cad.= $this->buscador[$a]["campo"]." = '".$fecha."'";
                            break;
                    }
                }
            }
    ...
    function ver() {
        ...
        if(!$_POST["tipo"]) {
            ...
            /** definimos accion_activa*/
            $r.='<input type="hidden" id="accion_activa" name="accion_activa">';
        ...
        if (!$this->datos->vacio() ) {
            if(!$_POST["tipo"]) {
                ...
                if ($this->atributos["ver_barra"]=="") {
                    ...
                    if($this->atributos["ver_imprimir"]=="1") {
                        $r.='<input type="button" value="Imprimir Reporte" class="grid_bar" title="Imprimir Reporte" style="width:86px;" id="btn_imprimir_grilla" />';
                    }
            ...
            //agregar sueltos - cabeceras
            if ($this->atributos["ver_estado_devuelto"]=="1")
            {
                $r.="<td width=13.2% height=21 valign=middle align=center class=grid_header>Envase Repuesto</td>";
            }
            ...
            while ($this->datos->no_vacio())
            {
                ...
                if ($this->atributos["ver_modificar"]=="")
                {
                    ...
                    $fecha = $this->datos->valor("fecha");
                    if(isset($this->atributos['tabla_re']))
                        //$r.='<td valign=middle align=center><a href="#" onClick="grid_imprimir(\''.$fecha.'\')"><img border=0 src="images/application_form_edit.png" width="16" height="16"></a> </td>';
                        //$r.='<td valign=middle align=center><a href="#" onClick="grid_imprimir(\''.$this->datos->valor("id").'\')"><img border=0 src="images/application_form_edit.png" width="16" height="16"></a> </td>';
                ...
                if ($this->datos->valor("estado") ==0 )
                {
                    ...
                    if($_SESSION["tipo"] !=1)
                    {
                        $aref='<a name="reg'.$this->datos->valor("id").'"><img border=0 src="images/inactivo.png"></a>';
                    }
                    else
                    {
                        $aref='<a name="reg'.$this->datos->valor("id").'"></a><a href="#reg'.$this->datos->valor("id").'" onClick="grid_toggle_estado_activa('.$this->datos->valor("id").')"><img border=0 src="images/inactivo.png"></a>';
                    }
                    ...
                    if($_SESSION["tipo"] !=1)
                    {
                        $aref='<a name="reg'.$this->datos->valor("id").'"><img border=0 src="images/activo.png"></a>';
                    }
                    else
                    {
                        $aref='<a name="reg'.$this->datos->valor("id").'"></a><a href="#reg'.$this->datos->valor("id").'" onClick="grid_toggle_estado_desactiva('.$this->datos->valor("id").')"><img border=0 src="images/activo.png"></a>';
                    }
                }
                ...
                if ($this->atributos["ver_eliminar"]=="")
                {
                    if($_SESSION["tipo"] !=1)
                    {
                        $r.='<td valign="middle" align="center"><img border=0 src="images/cross.png"></td>';
                    }
                    else
                    {
                ...
                if ($this->atributos["ver_nro"]=="")
                {
                    $r.=" <td height=21 valign=middle align=center class=\"form_centrar itemordenar\" onClick=' grid_modificar(".$this->datos->valor("id").")' style='cursor:pointer'>".$ord."</td>";
                }
                ...
                for ($a=0; $a<count($this->columnas);$a++)
                {
                    ...
                    //$r.=" <td class=\"col".$this->columnas[$a]["campo"]."\" height=21 valign=middle ".(($this->atributos["ver_modificar"]=="")?' onclick="grid_modificar('.$this->datos->valor("id").')" style="cursor:pointer"':"")." ".$css_celda.">&nbsp;".utf8_encode($this->datos->valor($alias))."</td> ";
...
//http://img.youtube.com/vi/VIDEO_ID/0.jpg
                    //comprobamos si el campo es una imagen para mostrar el thumbail de la imagen
                    if( ($this->columnas[$a]["campo"] != "imagen") && ($this->columnas[$a]["campo"] != "imagen_tipo1_esp") && ($this->columnas[$a]["campo"] != "imagen_tipo1_ing") && ($this->columnas[$a]["campo"] != "imagen_tipo1_frn") && ($this->columnas[$a]["campo"] != "imagen_tipo1_ita"))
                    {
                        if(($this->columnas[$a]["campo"] != "link_youtube"))
                            $r.=" <td class=\"col".$this->columnas[$a]["campo"]."\" height=21 valign=middle ".(($this->atributos["ver_modificar"]=="")?' onclick="grid_modificar('.$this->datos->valor("id").')" style="cursor:pointer"':"")." ".$css_celda.">&nbsp;".$this->datos->valor($alias)."</td> ";
                        else
                            $r.=" <td style='text-align: center;' class=\"col".$this->columnas[$a]["campo"]."\" height=21 valign=middle ".(($this->atributos["ver_modificar"]=="")?' onclick="grid_modificar('.$this->datos->valor("id").')" style="cursor:pointer"':"")." ".$css_celda.">&nbsp;<img src='http://img.youtube.com/vi/".$this->datos->valor($alias)."/0.jpg' width='60px' height='auto'/></td> ";
                    }
                    else
                    {
                        if($this->datos->valor($alias) != "")
                        {
                            $r.=" <td style='text-align: center;' class=\"col".$this->columnas[$a]["campo"]."\" height=21 valign=middle ".(($this->atributos["ver_modificar"]=="")?' onclick="grid_modificar('.$this->datos->valor("id").')" style="cursor:pointer"':"")." ".$css_celda.">&nbsp;<img src='recursos/images/60_33_".$this->datos->valor($alias)."' width='60px' height='auto'/></td> ";
                        }
                        else
                            $r.=" <td style='text-align: center;' class=\"col".$this->columnas[$a]["campo"]."\" height=21 valign=middle ".(($this->atributos["ver_modificar"]=="")?' onclick="grid_modificar('.$this->datos->valor("id").')" style="cursor:pointer"':"")." ".$css_celda.">&nbsp; - </td> ";
                    }
                }
		...
		for ($a=1;$a<=$this->contador_esp;$a++)
		{
            ...
            if(!$_POST["tipo"]) {
                ...
                for ($a=1;$a<=$t_pag;$a++) {
                    ...
                    $r.='<div class="divrpp"><select id="rpp" onchange="ir_paginacion()" onselect="ir_paginacion()" name="rpp" disabled="">'.$rpp.'</select>Registros por página</div>&nbsp;Mostrando del <span id="pdesde">'.$regdesde.'</span> al <span id="phasta">'.$reghasta.'</span> de <span id="pde">'.$this->datos->l_nroreg.'</span> registros.<img id="cargandoregistros" src="images/loading.gif" />';
                }
                $r.="</td></tr>";
    ...
    function ver_js() {
        $url=($this->atributos["url"]!="")?$this->atributos["url"]:$_SERVER['REQUEST_URI'];
        ...
        $r.="		f.submit();";
        $r.="	} ";
        $r.= " function grid_imprimir(fecha) { ";
        $r.= "  	var f=document.form_grid; ";
        $r.="	    f.action='imprime_reporte.php';";
        $r.="	    f.accion.value=20;";
        $r.="	    f.idobj.value=fecha;";
        //$r.="	    alert(f.action)";
        $r.="		f.submit();";
        $r.="	} ";
        ...
        $r.="		f.submit();";
        $r.="	} ";
        $r.= " function grid_modificar_pedido(id_pago, id) { ";
        $r.= "  	var f=document.form_grid_pedido; ";
        $r.="	    f.action ='".$url_form."';";
        $r.="	    f.accion.value=21;";
        //$r.="	    alert(id_pedido);";
        $r.="	    f.idobj.value=id_pago;";
        $r.="	    f.id_pedido_recibido.value=id;";
        //$r.="	    alert(g.id_pedido_recibido.value);";
        $r.="		f.submit();";
        $r.="	} ";
        $r.= " function grid_modificar_detpedido(id_pago, id, id_producto) { ";
        $r.= "  	var f=document.form_grid_pedido; ";
        $r.="	    f.action ='".$url_form."';";
        $r.="	    f.accion.value=21;";
        //$r.="	    alert(id_pedido);";
        $r.="	    f.idobj.value=id_pago;";
        $r.="	    f.id_pedido_recibido.value=id;";
        $r.="	    f.id_productos.value=id_producto;";
        //$r.="	    alert(g.id_pedido_recibido.value);";
        $r.="		f.submit();";
        $r.="	} ";
        ...
        $r.="	  {"."\n";
        $r.="	    f.action ='".$url."';";
        $r.="       if(/galeria/.test('".$url."')){";
        $r.="       f.accion.value=52;}else{";
        $r.="	    f.accion.value=8;}";
        $r.="	    f.idobj.value=id;";
        $r.="		f.submit();";
        $r.="	   }"."\n";
        ...
        $r.= "    f.action ='".$url."';"."\n";
        $r.= "     f.accion.value=12;"."\n";
        $r.= "     f.accion_activa.value=3;"."\n";
        $r.= "     f.submit();"."\n";
        $r.= "}"."\n";

        /** de prueba */
        $r.= "function grid_toggle_estado_desactiva(id){"."\n";
        $r.= "    var f=document.form_grid;"."\n";
        $r.= "    f.action ='".$url."';"."\n";
        $r.= "     f.idobj.value=id;"."\n";
        $r.= "     f.accion_activa.value=0;"."\n";
        $r.= "     f.accion.value=12;"."\n";
        $r.= "     f.submit();"."\n";
        //$r.= "     alert(f.idobj.value)"."\n";
        //$r.= "     alert(f.accion_activa.value)"."\n";
        $r.= "}"."\n";

        $r.= "function grid_toggle_estado_activa(id){"."\n";
        //$r.= "     $('#accion_activa').val(1)"."\n";
        //$r.= "     alert($('#accion_activa').val())"."\n";
        $r.= "    var f=document.form_grid;"."\n";
        $r.= "    f.action ='".$url."';"."\n";
        $r.= "     f.idobj.value=id;"."\n";
        $r.= "     f.accion_activa.value=1;"."\n";
        $r.= "     f.accion.value=12;"."\n";
        $r.= "     f.submit();"."\n";
        //$r.= "     alert(f.accion_activa.value)"."\n";
        $r.= "}"."\n";
