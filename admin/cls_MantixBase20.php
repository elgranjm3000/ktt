...
//var $tabla;
    ...
    function insertar() {
        ...
        if(isset($_POST['error_cantidad']))
        {
            $this->muestra_error_consignacion();
            return "El Stock Actual es inferior al ingresado. No puede ingresarse esa cantidad";
            break;
        }
        else
        {
            ...
            switch ($this->datos->c_error) {
                case 0: return "Inserci&oacute;n realizada con &eacute;xito";
                    break;
    ...
    function muestra_error_consignacion()
    {
        //return "No tiene Stock Sufiente para Consignar ese Producto. Deberia Registrar un Nuevo Pedido";
        //break;
    }
    
    function actualizar_actualizar_orden()
    {
        if($_POST['orden'] != "")
        {
            /** actualizamos la ubicacion de los otros */
            $query = "update mor_galeria set orden = orden + 1 where orden >= ".$_POST['orden']." and orden < ".$_POST['orden_ant']."";
            $res = $this->datos->ejecutar($query);
            
            $query = "update mor_galeria set orden = ".$_POST['orden']." where id = ".$_POST['idobj']."";
            $res = $this->datos->ejecutar($query);
        }
    }
    
    function eliminar_simple_actualizar_orden()
    {
        /** obtenemos el orden del id a eliminar */
        $this->datos->listaSP("select orden from mor_galeria where id = ".$_POST['idobj']."", "", "");
        while($this->datos->no_vacio()) 
        {
            $orden = $this->datos->valor("orden");
        }
        
        /** actualizamos la ubicacion de los otros */
        $query = "update mor_galeria set orden = orden - 1 where orden >= ".$orden." ";
        $res = $this->datos->ejecutar($query);
        
        /** llamamos al meotod encargado de eliminar */
        $this->eliminar_simple();
        //echo "si";
    }
    ...
    {
        $sql = "select mcl.id as id from mor_pedidos mcp, mor_compras mco, mor_clientes mcl where mcp.id_compra = mco.id and mco.id_usuarios = mcl.id and mcp.id = ".$_POST['idobj']."";
        $c = new MantixOaD();
        $c->listaSP($sql, "", "");
        while($c->no_vacio()) 
        {
            $id_cliente = $c->valor("id");
        }
        
        
        $sql = "update mor_clientes set firstname = '".$_POST['firstname']."', lastname = '".$_POST['lastname']."', dni = '".$_POST['dni']."', phone = '".$_POST['phone']."', address = '".$_POST['address']."', city = '".$_POST['localidad']."', zipcode = '".$_POST['zipcode']."', state = ".$_POST['city'].", country = ".$_POST['country'].", billing_cost = ".$_POST['billing_cost']." where id = ".$id_cliente."";  
        $result = $c->ejecutar($sql);
        
        //$sql = "update mor_pedidos set "
    ...
    function grid_toggle_estado() {
        ...
        //return $_POST["accion_activa"];
        if($_POST["accion_activa"]== 3)
        {
            $this->datos->toggle_estado();
            return "Cambio de estado realizado con &eacute;xito";
        }
        ...
        {
            $this->datos->toggle_estado_actualizar_estado();
            //return "Cambio de estado realizado con &eacute;xito";
            return "";
        }
    }
